...
public static class DryRunCommand
{
    public static Command Build()
    {
        ...
        var existingPathOption = new Option<string?>("--existing-path");
        ...
        var command = new Command("dry-run", "Execute a dry-run that produces plan output without writes")
        {
            desiredPathOption,
            existingPathOption,
            keyPrefixOption,
        ...
        command.SetAction(async (parseResult, cancellationToken) =>
        {
            ...
            var existingPath = parseResult.GetValue(existingPathOption);
            ...
            var existingState = string.IsNullOrWhiteSpace(existingPath)
                ? new ExistingStateSnapshot()
                : Load<ExistingStateSnapshot>(existingPath!);
            ...
            var handler = new DryRunHandler(engine);
            var plan = await handler.ExecuteAsync(desiredState, existingState, cancellationToken);
